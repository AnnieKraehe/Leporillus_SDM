...
walk(seq_along(time_steps_bp), function(idx) {
  ...
  current_middens <- midden_data %>% 
    filter(time_step == round(year_bp, -3))
  ...
  # Prepare All Midden Points
  all_middens <- midden_data %>% 
  ...
  p <- ggplot() +
    ...
    # Add all points as white-filled dots with black outlines
    geom_point(data = all_middens, aes(x = longitude, y = latitude),
               fill = "white", color = "black", size = 1.5, shape = 21, stroke = 0.1) +
    # Add current timestep points as pink-filled dots with black outlines
    geom_point(data = current_middens, aes(x = longitude, y = latitude),
               fill = "pink", color = "black", size = 1.5, shape = 21, stroke = 0.1) +
    ggtitle(paste0("Krapp2021 Apicalis + middens Years BP: ", round(year_bp, -3))) +
    theme_minimal()
  
  # Save the Plot
  ggsave(filename = paste0("C:/github/SNR_SDM/Results/SDM maps with middens/new pseudo absence/apicalis_map_", 
                           round(year_bp, -3), ".png"),
         plot = p,
         width = 7, height = 7)
})
