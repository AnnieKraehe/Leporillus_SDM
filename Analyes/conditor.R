...
#   add_member(conditor_models, metric = "boyce_cont")
# 
...
i=0
for (i in seq(from=-1000, to=-40000,  by=-1000)){
  print(paste("Processing time BP", i))
  file_name <- sprintf("%d_conditor.jpg", i)
...
# plot<- plot(prediction_lgm, main =file_name)


# Check if the geometries are polygons and handle accordingly
#final_plot<plot(st_geometry(australia_shape), col = NA, border = 'red', lwd = 2, add = TRUE) 
#else {
#coords <- st_coordinates(st_geometry(australia_shape))
#lines(coords, col = 'red', lwd = 2)

#add outline to final plot
#final_plot<lines(st_geometry(australia_shape), col = 'red', lwd = 2)


# Save the plot as a JPEG file

ggsave(filename = file_name, plot = final_plot, path = "C:/github/SNR_SDM/Results/conditor_SDM", device = "jpeg", width = 10, height = 8, units = "in")
