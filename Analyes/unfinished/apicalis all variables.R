...
library(pastclim)
climate_vars <- c("bio01", "bio04", "bio05", "bio06","bio07","bio08", "bio09", "bio10", "bio11","bio12", "bio13", "bio14","bio15","bio16", "bio17", "bio18","bio19")
...
apicalis_ensemble <- simple_ensemble() %>% add_member(apicalis_models, metric = "tss_max")
...
lines(st_geometry(australia_shape), col = 'red', lwd = 2)



plot(australia_shape$geometry, col = NA,  border = "black", lwd = 0.5)
rast_template <- rast(nrows=100, ncols=100, ext=ext(australia_shape))  # Adjust the resolution as needed

# Rasterize the vector
Aust_raster <- rasterize(australia_shape, rast_template)


# Print current CRS of both objects
crs(prediction_lgm)  # Terra package
crs(australia_shape) # SF package uses st_crs


  australia_shape <- st_transform(australia_shape, crs = crs(prediction_lgm))
  # Plot the raster
  plot(prediction_lgm, main = "Overlay of Vector and Raster Data", col = terrain.colors(100))
  
  # Overlay the vector data
  plot(australia_shape, add = TRUE, col = NA,  border = 'red', lwd = 2)
