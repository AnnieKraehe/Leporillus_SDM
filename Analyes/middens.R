...
middens <- st_as_sf(middens, coords = c("longitude", "latitude"))
st_crs(middens) <- 4326
...
ggplot() +
  ...
  geom_sf(data = middens, aes(col = time_bp))
...
set.seed(123)
middens <- sample_pseudoabs_time(middens,
                                n_per_presence = 3,
                                raster = climate_full,
                                time_col = "time_bp",
...
ggplot() +
  ...
  scale_fill_terrain_c() +
  geom_sf(data = middens, aes(col = class))
...
#Now let’s get the climate for these location. pastclim requires a data frame with two columns with coordinates and a column of time in years before present (where negative values represent time in the past). We manipulate the sf object accordingly:
  ...
  middens_df <- middens %>%
  dplyr::bind_cols(sf::st_coordinates(middens)) %>%
  mutate(time_bp = date2ybp(time_step)) %>%
  as.data.frame() %>%
  select(-geometry)
# get climate
  middens_df <- location_slice_from_region_series(middens_df,
...
# as we don't need it for modelling
  middens <- middens %>%
  bind_cols(middens_df[, climate_vars]) %>%
  select(-time_step)
...
middens_models <-
  ...
  workflow_set(
    preproc = list(default = middens_rec),
    models = list(
  ...
  update_workflow_model("default_gam",
                        spec = sdm_spec_gam(),
                        formula = gam_formula(middens_rec)
  ) %>%
...
#We now want to set up a spatial block cross-validation scheme to tune and assess our models:
  ...
  library(tidysdm)
set.seed(1005)
middens_cv <- spatial_block_cv(middens, v = 5)
autoplot(middens_cv)
...
middens_ensemble <- simple_ensemble() %>%
  add_member(middens_models, metric = "boyce_cont")

autoplot(middens_ensemble)
...
prediction_lgm <- predict_raster(middens_ensemble, climate_lgm)
ggplot() +
